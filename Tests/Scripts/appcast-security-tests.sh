...
preserve_old=0
if [[ -f "$output" ]] && grep -Fq '<sparkle:version>6</sparkle:version>' "$output"; then
  preserve_old=1
fi
...
XML
  if [[ "$preserve_old" == "1" ]]; then
    cat <<'XML'
    <item><sparkle:version>6</sparkle:version><sparkle:shortVersionString>0.0.9</sparkle:shortVersionString><enclosure url="https://github.com/stcheng/shuo/releases/download/v0.0.9/Shuo-0.0.9-macOS.zip" length="1" sparkle:edSignature="old-signature" /></item>
XML
  fi
  for url in "${current_urls[@]}"; do
    printf '    <item><sparkle:version>7</sparkle:version><sparkle:shortVersionString>0.1.0</sparkle:shortVersionString><enclosure url="%s" length="%s" sparkle:edSignature="test-signature" /></item>\n' \
      "$url" "$archive_length"
  done
  cat <<'XML'
  </channel>
...
grep -Fq '<sparkle:version>6</sparkle:version>' "$APPCAST_OUTPUT" \
