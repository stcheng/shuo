...
# Sparkle writes version metadata as children of <item>, not attributes of its
# <enclosure>. Bind all checks to that one item so a valid generated feed is
# accepted while a same-version entry at a different URL is still rejected.
MATCHING_ITEM_XPATH="//*[local-name()='item' and *[local-name()='enclosure' and @url='$EXPECTED_ARCHIVE_URL'] and *[local-name()='version' and normalize-space(.)='$BUILD_NUMBER'] and *[local-name()='shortVersionString' and normalize-space(.)='$VERSION']]"
MATCHING_ITEM_COUNT="$(xmllint --xpath \
  "count($MATCHING_ITEM_XPATH)" \
  "$WORK_DIR/appcast.xml")"
[[ "$MATCHING_ITEM_COUNT" == "1" ]] \
  || fail "Canonical appcast enclosure does not bind version $VERSION build $BUILD_NUMBER exactly once."
CURRENT_VERSION_BUILD_XPATH="//*[local-name()='item' and *[local-name()='version' and normalize-space(.)='$BUILD_NUMBER'] and *[local-name()='shortVersionString' and normalize-space(.)='$VERSION']]"
CURRENT_VERSION_BUILD_COUNT="$(xmllint --xpath \
  "count($CURRENT_VERSION_BUILD_XPATH)" \
  "$WORK_DIR/appcast.xml")"
...
MATCHING_SIGNATURE="$(xmllint --xpath \
  "string(($MATCHING_ITEM_XPATH/*[local-name()='enclosure' and @url='$EXPECTED_ARCHIVE_URL'])[1]/@*[local-name()='edSignature'])" \
  "$WORK_DIR/appcast.xml")"
MATCHING_LENGTH="$(xmllint --xpath \
  "string(($MATCHING_ITEM_XPATH/*[local-name()='enclosure' and @url='$EXPECTED_ARCHIVE_URL'])[1]/@length)" \
  "$WORK_DIR/appcast.xml")"
